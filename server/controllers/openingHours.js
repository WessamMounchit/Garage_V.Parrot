  ...
  try {
    for (const item of openingHoursData) {
      const query = `UPDATE opening_hours 
                     ...
                     WHERE day = $5`;
      const values = [item.morning_open, item.morning_close, item.afternoon_open, item.afternoon_close, item.day];
    ...
    console.error(error);
    res.status(500).json({ error: 'Erreur lors de la mise à jour des horaires d\'ouverture' });
  }};
