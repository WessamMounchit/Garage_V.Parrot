...
const Footer = () => {
  ...
  const formatOpeningHours = (openingHour) => {
    ...
    const isAfternoonOnly =
      !morning_open && !morning_close && afternoon_open && afternoon_close;
    const isFullDay =
      morning_open && afternoon_close && !morning_close && !afternoon_open;
    const isFullDayWithBreak =
      morning_open && morning_close && afternoon_open && afternoon_close;
    const isClosed =
      !morning_open && !afternoon_close && !morning_close && !afternoon_open;
      ...
      <div key={day}>
        <p>
          ...
          {isFullDayWithBreak && (
            <span>
              {" "}
              {morning_open} - {morning_close}, {afternoon_open} -{" "}
              {afternoon_close}
            </span>
          )}
          {isFullDay && (
          ...
          <Row>
            <Col lg="4" md="5" sm="12">
              ...
              <p className="footer__logo-content">
                Découvrez le Garage Vincent Parrot et plongez dans l'univers
                automobile exceptionnel du Garage Vincent Parrot. Avec une
                passion pour les véhicules qui traverse les générations, nous
                sommes là pour vous offrir des services de réparation,
            ...
            >
              <div className="mb-4">
                ...
                <span className="office__info">{content}</span>
              </div>
            </Col>
